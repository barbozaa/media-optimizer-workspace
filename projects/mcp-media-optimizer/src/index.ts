...
function applySharpFormat(img: sharp.Sharp, format: ImageFormat, quality: number): sharp.Sharp {
  switch (format) {
...
async function encodeWithBudget(
  ...
  const encode = (q: number): Promise<Buffer> =>
    applySharpFormat(sharp(inputBuffer), format, q).toBuffer();
  ...
  let fittingBuffer: Buffer | null = null;
  ...
  if (fittingBuffer) return { buffer: fittingBuffer, quality: fittingQuality };
...
async function getImageInfo(imagePath: string): Promise<ImageInfo> {
 ...
 */
function suggestFormat(info: ImageInfo): { format: ImageFormat; reason: string } {
  if (info.hasAlpha) {
    return {
      format: 'avif',
 ...
 */
function estimateSize(info: ImageInfo, quality: number, targetFormat: ImageFormat): number {
  const COMPRESSION_FACTORS: Record<ImageFormat, number> = {
    webp: 0.65, avif: 0.50, png: 0.75, jpeg: 0.85,
  };
  const PNG_EXPANSION: Record<ImageFormat, number> = {
...
async function compressImage(
  ...
  const { buffer, quality: qualityUsed } = await encodeWithBudget(
    decoded, outputFormat, quality, maxBytes,
  );
...
function buildOutputPath(inputPath: string, outputFormat: ImageFormat, outputDir?: string): string {
...
const server = new Server(
  { name: 'mcp-media-optimizer', version: '1.0.0' },
  { capabilities: { tools: {} } },
);
  ...
  tools: [
    {
      name: 'compress_image',
      description:
        'Compress or convert a single image file (JPEG, PNG, WebP, AVIF). ' +
    ...
    },
    {
      name: 'analyze_image',
      description:
        'Analyze an image: dimensions, format, size, transparency, animation status, ' +
    ...
    },
    {
      name: 'compress_batch',
      description:
        'Compress or convert multiple images in parallel. ' +
    ...
    },
    {
      name: 'suggest_format',
      description:
        'Suggest the optimal output format for an image based on its content: ' +
    ...
    },
    {
      name: 'estimate_size',
      description:
        'Estimate the compressed file size for a given quality and target format ' +
  ...
  try {
    switch (name) {
      ...
      case 'compress_image': {
        ...
        const info = await getImageInfo(inputPath);
        const outputFormat = (args?.outputFormat as ImageFormat | undefined) ?? info.format;
        ...
        const result = await compressImage(inputPath, outputPath, {
          ...
          maxWidthOrHeight: args?.maxWidthOrHeight as number | undefined,
        });
      ...
      case 'analyze_image': {
        ...
        const estimatedSizes: Record<string, string> = {};
        for (const fmt of ['webp', 'jpeg', 'png', 'avif'] as ImageFormat[]) {
      ...
      case 'compress_batch': {
        ...
        if (!inputPaths?.length) throw new Error('inputPaths must be a non-empty array');

        const concurrency = (args?.concurrency as number | undefined) ?? 4;
        const outputDir   = args?.outputDir as string | undefined;
        const outputFormat = args?.outputFormat as ImageFormat | undefined;
      ...
      case 'estimate_size': {
        ...
        const targetFormat = (args?.targetFormat as ImageFormat | undefined) ?? info.format;
